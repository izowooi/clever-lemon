...
class HttpAuthApiService implements AuthApiService {
  ...
  @override
  Future<ApiResult<Map<String, dynamic>>> withdraw(WithdrawRequest request) async {
    try {
      final url = Uri.parse(ApiConfig.authWithdrawUrl);
      final response = await http.post(
        url,
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        return ApiResult.success(
          '탈퇴 처리가 완료되었습니다. 2주 유예 기간 후 계정이 삭제됩니다.',
          data: responseBody,
          statusCode: response.statusCode,
        );
      } else {
